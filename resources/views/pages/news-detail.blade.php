...
<div class="py-12 px-4 max-w-5xl mx-auto space-y-12">
    ...
    <div class="aspect-video rounded-[2.5rem] overflow-hidden shadow-2xl border border-slate-100 relative group">
        <img src="{{ asset('storage/' . $news->image) }}" alt="{{ $news->title }}" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105">
    </div>
    @endif
    <!-- Article Content -->
                ...
                <div class="space-y-8">
                    ...
                    <a href="{{ route('news.show', $related->slug) }}" class="flex gap-4 group">
                        <div class="w-20 h-20 shrink-0 rounded-2xl overflow-hidden bg-slate-100">
                            @if($related->image)
                                <img src="{{ asset('storage/' . $related->image) }}" alt="{{ $related->title }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform">
                            @else
                                <div class="w-full h-full bg-emerald-100 flex items-center justify-center text-emerald-600 font-bold text-xs">
                                    {{ substr($related->title, 0, 2) }}
                                </div>
                            @endif
                        </div>
                        <div class="space-y-1">
                            <h4 class="text-sm font-bold text-slate-900 leading-tight group-hover:text-emerald-600 line-clamp-2">{{ $related->title }}</h4>
                            <p class="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{{ \Carbon\Carbon::parse($related->published_at)->translatedFormat('d M Y') }}</p>
                        </div>
                    </a>
                    @endforeach
                </div>
