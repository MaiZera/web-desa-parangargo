                            ...
                            <div class="space-y-6">
                                ...
                                <div>
                                    ...
                                    <textarea name="deskripsi" id="deskripsi" rows="4"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 sm:text-sm">{{ old('deskripsi', $umkm->deskripsi) }}</textarea>
                                ...
                                <div>
                                    ...
                                    <div id="produk-container" class="space-y-2">
                                        @php
                                            $produkList = old('produk_layanan', $umkm->produk_layanan ?? ['']);
                                            // Handle case if data is somehow not array
                                    ...
                                    <div class="grid grid-cols-2 gap-4 mt-1">
                                        <div>
                                            <input type="number" name="kisaran_harga_min"
                                                value="{{ old('kisaran_harga_min', $umkm->kisaran_harga_min) }}"
                                                placeholder="Min"
                                                class="block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 sm:text-sm">
                                        </div>
                                        <div>
                                            <input type="number" name="kisaran_harga_max"
                                                value="{{ old('kisaran_harga_max', $umkm->kisaran_harga_max) }}"
                                                placeholder="Max"
                                                class="block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 sm:text-sm">
                                        </div>
                                    </div>
                                ...
                                <div>
                                    ...
                                    @php
                                        ...
                                        foreach ($days as $day) {
                                            ...
                                            // Ensure we check null coalesce correctly
                                            $currentBuka = $current['buka'] ?? '';
                                            $firstBuka = $firstDay['buka'] ?? '';
                                            $currentTutup = $current['tutup'] ?? '';
                                            $firstTutup = $firstDay['tutup'] ?? '';
                                            $currentLibur = $current['libur'] ?? false;
                                            $firstLibur = $firstDay['libur'] ?? false;

                                            if (
                                                $currentBuka !== $firstBuka ||
                                                $currentTutup !== $firstTutup ||
                                                $currentLibur != $firstLibur
                                            ) {
                                                $isSimpleMode = false;
                                                break;
                                            }
                                        ...
                                        $simpleTutup = $firstDay['tutup'] ?? '';
                                        $simpleLibur = $firstDay['libur'] ?? false;
                                    @endphp
                                    ...
                                    <!-- Detailed Mode Input -->
                                    <div id="schedule-detail" class="space-y-2 {{ $isSimpleMode ? 'hidden' : '' }}">
                                        @foreach($days as $day)
                                            @php
                                                $schedule = $umkm->jam_buka[$day] ?? [];
                                                $buka = old("jam_buka.$day.buka", $schedule['buka'] ?? '');
                                                $tutup = old("jam_buka.$day.tutup", $schedule['tutup'] ?? '');
                                                $libur = old("jam_buka.$day.libur", $schedule['libur'] ?? false);
                                            @endphp
                            ...
                            <div class="space-y-6">
                                <div>
                                    ...
                                    <textarea name="alamat" id="alamat" rows="2" required
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 sm:text-sm">{{ old('alamat', $umkm->alamat) }}</textarea>
                                ...
                                <div class="pt-4">
                                    <div class="flex items-start">
                                        <div class="flex h-5 items-center">
                                            <input id="is_active" name="is_active" type="checkbox" value="1" {{ old('is_active', $umkm->is_active) ? 'checked' : '' }}
